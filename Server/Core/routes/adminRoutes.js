...
router.get("/api/admin/users", async (req, res) => {
    ...
    const paginatedUsers = await User.find({}).select("-password").populate('roles', 'name grade').skip((page - 1) * itemsPerPage).limit(itemsPerPage).lean()

    const users = await User.find({}).select("-password").populate("roles", "name grade").lean()
...
router.get("/api/admin/users/:slug", async (req, res) => {
    ...
    const user = await User.findOne({slug}).select("-password").populate('roles', 'name grade')
    console.log(user);
