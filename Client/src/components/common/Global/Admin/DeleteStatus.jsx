...
const DeleteStatus = ({ selectedStatus }) => {
          ...
          <div className="fixed z-20 top-1/2 left-1/2 w-[500px] -translate-x-1/2 -translate-y-1/2 bg-adminBgAlt rounded-xl p-[20px] border border-zinc-800">
            <div className="justify-between items-center flex">
              <h3 className="text-zinc-100 font-medium text-[20px] tracking-tight">
                {selectedStatus.length > 1 ? "Suppression de status" : "Suppression de statut"}
              </h3>
              <Close
            ...
            <div className="bg-zinc-900 w-full py-3 px-2 rounded-md mt-3">
                <div>
                    <h4 className="mt-1 text-zinc-300">
                        {selectedStatus.length > 1 ? `Status sélectionnés :` : `Statut sélectionné :`}
                    </h4>
                    {selectedStatus.map((status, index) => (
                        <h4 key={index} className="text-[15px] text-zinc-300">- {status}</h4>
                    ))}
                </div>
            </div>
            <form className="mt-7" onSubmit={handleSubmit}>
              ...
              >
                {isLoading ? "Suppression..." : (
                    selectedStatus.length > 1 ? "Supprimer les status" : "Supprimer le statut"
                )}
              </Button>
            </form>
          </div>
